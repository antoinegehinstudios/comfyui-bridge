...
| une IMAGE FIXE livrée (sa taille exacte, un message posé dessus, un logo, une texture) | l'étape `composer` de la chaîne : `image`, `largeur`, `hauteur`, et les MÊMES clés `textes` / `images` / `texture` qu'au recollage (une image n'a pas de temps : minutage et fondu y sont sans effet ; `decalage` pose un sous-titre sous le titre) | la livraison (`adapter/composition_image.py`, qui appelle `textes.py` et `incrustations.py`) ; la fiche porte `cadrage`, `textes_poses`, `textes_dits[]`, `images_posees` |
