...
| un CHAMP dont la VALEUR est IMPOSÉE par un autre (la palette, la police, la couleur et le fond des titres, le négatif — par la charte), qui doit rester à l'écran, grisé, avec ce qui l'impose (Antoine, 2026-09-24 : « grise les champs imposés au lieu de les cacher ; fais-en un standard ») | la clé `impose_par: {"champ": "charte", "sauf": ["aucune"]}` du champ (chaîne ou technique ; noyau `Champ.impose_par`, refusée si le maître n'existe pas ou est le champ lui-même) ; ce que chaque valeur du maître impose vient du FOURNISSEUR du menu (colonne « impose », projetée par `menus.<nom>.source_fichier.impose: {"colonne", "champs": {clé du fournisseur: champ de la chaîne}}` — Héraldiste parle sa langue, la réconciliation traduit) | `/io` (`impose_par` sur le champ, `choix[].impose` sur le maître) ; le lanceur grise le champ, montre la valeur imposée, ne l'envoie pas — et, quand le choix du maître DIT ce qu'il impose, ne grise que ce qu'il nomme |
