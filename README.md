...
| QUAND une étape a lieu — et ce qu'elle rend quand elle n'a pas lieu | la clé `quand` de l'étape : un renvoi (« joue-la si ce champ n'est pas vide »), ou une condition NOMMÉE `{"valeur": "$champ", "op": "ne", "attendu": "…"}` — les mots d'un contrôle, pour ce qu'un renvoi seul ne sait pas dire : une valeur de menu qui signifie « rien à faire » n'est jamais vide. Et `sinon`, RÉSOLU comme le reste : une étape de passe-plat y rend `"$prompt"` tel quel | le runner de chaînes ; l'étape sautée porte `sautee`, sa `raison` et `sans_effet[]` |
...
| une IMAGE FIXE livrée (sa taille exacte, un message posé dessus, un logo, une texture) | l'étape `composer` de la chaîne : `image`, `largeur`, `hauteur`, et les MÊMES clés `textes` / `images` / `texture` qu'au recollage (une image n'a pas de temps : minutage et fondu y sont sans effet ; `decalage` décale un texte, `sous_le_precedent: true` le pose SOUS le texte posé avant lui à la même position, à la hauteur réelle de son bloc — le groupe remonte s'il déborde) | la livraison (`adapter/composition_image.py`, qui appelle `textes.py` et `incrustations.py`) ; la fiche porte `cadrage`, `textes_poses`, `textes_dits[]`, `images_posees` |
| un CHAMP qui n'a de sens que sous certaines valeurs d'un AUTRE champ de la chaîne, ou HORS de certaines (le logo d'une charte n'existe que sous une charte, quelle qu'elle soit) | la clé `selon: {"champ": "charte", "valeurs": [...]}` du champ, ou `selon: {"champ": "charte", "sauf": ["aucune"]}` (depuis le 2026-09-24 au soir), dans `expose` de la chaîne — l'une ou l'autre, non vide ; refusée si elle désigne un champ absent ou le champ lui-même | `/io` (`selon`, la même clé que pour un réglage de technique) ; le lanceur ne montre le champ que sous ces valeurs (ou hors de celles-là) et ne l'envoie pas sinon |
| une VALEUR qui dépend d'un fait qu'une étape d'amont livre (la zone calme que la direction demande au modèle n'existe que si le nœud de la charte pose un logo pendant l'image) | une valeur conditionnelle, n'importe où dans les paramètres d'une étape ou les entrées d'un rôle : `{"si": "$contrainte.recit.logo_fichier_pendant", "alors": "$contrainte.recit.logo_ancrage", "sinon": ""}` — `alors` quand ce que `si` désigne n'est pas vide (la règle de `quand`), `sinon` autrement ; les trois clés, `si` un renvoi, sinon refusée à la lecture (noyau `est_conditionnelle`, `resoudre`) ; c'est ainsi qu'une chaîne écrit l'USAGE d'un fait, chez le réalisateur | le runner ; l'aperçu montre la condition tant que `si` n'est pas connu |
| un CHAMP dont la VALEUR est IMPOSÉE par un autre (la palette, la police, la couleur et le fond des titres, le négatif — par la charte), qui doit rester à l'écran, grisé, avec ce qui l'impose (Antoine, 2026-09-24 : « grise les champs imposés au lieu de les cacher ; fais-en un standard ») | la clé `impose_par: {"champ": "charte", "sauf": ["aucune"]}` du champ (chaîne ou technique ; noyau `Champ.impose_par`, refusée si le maître n'existe pas ou est le champ lui-même) ; ce que chaque valeur du maître impose se déclare sur les FAITS du fournisseur du menu (une colonne de faits, dans sa langue — `faits` chez Héraldiste, schéma `heraldiste/menu`) : `menus.<nom>.source_fichier.impose: {"colonne": "faits", "champs": {<champ>: {"fait": "titres.famille", "si": "titres.fichier", "dit": "un fond plein {valeur} sous le texte"}}}` — quel champ, sous quelle condition, en quels mots : l'USAGE est ici, chez le réalisateur ; le fournisseur ne le dit jamais (Antoine, 2026-09-24 : « Héraldiste expose des éléments factuels, l'usage qui en est fait est propre au réalisateur ») ; un fait vide n'impose rien | `/io` (`impose_par` sur le champ, `choix[].impose` sur le maître) ; le lanceur grise le champ, montre la valeur imposée, ne l'envoie pas — et, quand le choix du maître DIT ce qu'il impose, ne grise que ce qu'il nomme |
| ce qu'une TECHNIQUE ne règle PAS — sans jamais un faux champ à la place (Antoine, 2026-09-24 : « s'il n'y a rien dans ComfyUI en ce sens, on ne met pas de faux champ ; ce manque peut être mentionné quand il est détecté ; la règle doit compter partout ») | DÉTECTÉ par la passerelle (`adapter.techniques.manques`) parmi les techniques voisines de la même chaîne : un réglage qu'une voisine expose et qu'elle n'a pas — dit par l'auteur (section `<champ>: {"applique": false, "dit": …}` du fichier de la technique, `declare`), sinon lu dans son graphe quand une entrée de ce nom y est tenue à une valeur fixe (`graphe`, avec la valeur et le nœud), sinon `absent`. Une section `applique: false` sur un champ que la technique expose pourtant est refusée au chargement (contradiction) | `/io` (`manques_par_valeur` sur le champ qui choisit la technique : `{valeur: [{quoi, libelle, detecte, dit}]}`), `/v1/workflows` (`techniques[].manques`) ; le lanceur le dit sous le champ, une ligne par manque |
| un champ EXPOSÉ que rien ne lit (il s'afficherait, se remplirait, ne pèserait sur rien : un faux champ) | REFUSÉ au chargement, pour toute chaîne et toute technique (noyau `champs_que_rien_ne_lit`, dans `verifier_techniques`, après les rôles et les renvois) : un champ de chaîne qu'aucune étape ni aucune technique ne nomme, un champ de technique qu'aucun de ses rôles, contrôles ni aucune étape ne nomme sous cette chaîne — sauf le champ qui choisit la technique. Le brancher, ou le retirer | le journal du démarrage (le mode manque au catalogue, la cause nomme le champ) |
| le GABARIT d'un cas de création — les étapes, leur ordre, leurs genres et rôles, ce qui doit être constaté, le premier champ, ce que le maître impose — MANIFESTÉ par la chaîne et vérifié contre elle (Antoine, 2026-09-24 : « standardise par un template un cas de création, en manifestant le template à suivre ») | `resources/gabarits/<nom>.json` (le gabarit, lu par `core.gabarit`) et la clé `"gabarit": "<nom>"` de la chaîne ; la lecture humaine dans `resources/chaines-exemples/GABARIT-<nom>.md` — une chaîne qui manifeste un gabarit et s'en écarte est refusée au chargement, chaque écart nommé | `/v1/workflows` (`presentation.gabarit`) |
| UN RÉCONCILIANT PAR TECHNO — le seul élément qui lit une source extérieure (Héraldiste : `charte` ; Iconographe : `analyse` ; Iconologue : `culture`), avec ses champs, ses étapes et des emplacements par rôle ; une chaîne le branche et ne lit que `$<rôle>.<emplacement>` (Antoine, 2026-09-25 : « seul cet élément réconciliant se met à jour, une fois, pour que tout workflow obtienne la nouveauté ») | `resources/reconciliants/<rôle>.json`, lus et dépliés par `core.reconciliant` au chargement de chaque chaîne ; mode d'emploi `resources/reconciliants/LIRE-MOI.md` — une lecture directe d'une source (graphe, menu, récit de ses étapes) est refusée | `/v1/workflows` (`presentation.reconciliants` : version, emplacements lus, `non_pris`) ; `choix[].non_servis` (les faits qu'une source sert et que son réconciliant ne lit pas) |
| le SOCLE de toute création publiée — contrôle final du fichier livré, champs documentés, livrable déclaré, sources par leur réconciliant (« chaque nouveau flux de création sur maestro suit la même structure à la base ») | `resources/gabarits/socle.json`, que `creation` étend (`"etend"`) ; le catalogue le DÉCLARE (`"socle": "socle"`) : une chaîne publiée qui ne le suit pas est refusée au chargement | `/v1/workflows` (`presentation.gabarit`, `presentation.hors_socle` d'un graphe seul publié) |
| ce qu'une CHARTE impose à une IMAGE créée (palette à la place de celle choisie, mots de style, interdits, police et couleurs de titre, logo posé quand la création en porte un, texture, constat sur l'image) | la chaîne : l'étape `contrainte` EN PREMIER (graphe `image-heraldiste` ; `1.logo` = le champ `logo` — selon le message, avec, sans, dits par le menu NOMMÉ `logo_charte` de la réconciliation (`options_depuis`, depuis le 2026-09-25 : « Quand l'image porte un message ou une promotion », « Toujours, même sur une image d'ambiance », « Jamais, même avec un message », le principe dans le résumé de chacun ; nommé pour ne pas retitrer le champ `logo` d'une autre chaîne) ; le visuel social envoie `message` et `sous_message` comme `1.accroche` et `1.appel` : un message posé, la marque signe), la `direction` qui reçoit `positif`, `colorway_en` (les couleurs DOMINANTES — dominante, secondaire, accent — pas la palette entière, qui reste ce que le constat mesure), `negatif`, et la zone calme `logo_ancrage` SEULEMENT si le nœud pose un logo pendant l'image (valeur conditionnelle sur `logo_fichier_pendant`), la `livraison` (composer : `textes`, `images` — le logo = `logo_fichier_pendant`, sa `raison` = `logo_raison`, dite au journal et dans `images_non_posees` —, le nom que le wordmark écrit ôté du message SEULEMENT si le logo est posé (`laisser_au_logo` conditionnel ; sinon la marque disparaîtrait de l'image, vu le 2026-09-24 à 22:23), `texture`), l'étape `conformite` (graphe `video-heraldiste-conformite`, `1.video` = le chemin de l'image livrée, `1.images` = 1, `1.logo_attendu` = la décision : le logo n'est cherché que là où il est attendu), les constats de `constat_de_la_charte` ; et dans chaque technique `negatif.applique` (le négatif est-il lu ?) | le runner ; `DirectionDImage` (récit `charte_palette`, `palette_dite`, `zone_du_logo`) ; `HeraldisteCharte` (récit `positif`, `couleurs_en` et `logo_fichier` — les faits —, `colorway_en`, `logo_pose`, `logo_raison`, `logo_lieu`, `logo_attendu`, `logo_fichier_pendant` — la décision) |
| le STYLE d'une IMAGE créée (médium, cadrage, lumière, palette, ambiance) | les catalogues `styles/images.json`, `cadrages.json`, `lumieres.json`, `palettes.json`, `ambiances.json` du paquet de direction de style — chaque entrée porte les MOTS qu'elle ajoute (« libre » n'en ajoute aucun) ; une entrée éprouvée de plus, jamais un mot de style dans la chaîne | `/io` (menus `*_image`), `DirectionDImage` (récit `mots_par_parametre`) |
...
| une TECHNIQUE (quel graphe tient chaque rôle, ses réglages, ses contrôles, ce que son nœud IMPOSE — `budget.queue_s`) | `_data/techniques/<nom>.json` (et sa copie `resources/techniques-exemples/`) — une technique de plus est un FICHIER de plus ; le plan lit son fichier par `$technique.<chemin>`. Le dossier est UN pour toutes les chaînes : une chaîne ne voit que les techniques qui tiennent SES rôles (`deroulement`… pour une révélation, `image` pour une création), les autres sont d'un autre plan (depuis le 2026-09-24, `core.chaine.techniques_pour`) | le runner de chaînes, `/v1/workflows` (`techniques`), `/io` (`selon`) |
