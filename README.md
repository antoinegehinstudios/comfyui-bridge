...
| un CHAMP qui n'a de sens que sous certaines valeurs d'un AUTRE champ de la chaîne (la palette qu'on choisit s'efface devant celle d'une charte) | la clé `selon: {"champ": "charte", "valeurs": ["aucune"]}` du champ, dans `expose` de la chaîne — refusée si elle désigne un champ absent ou le champ lui-même | `/io` (`selon`, la même clé que pour un réglage de technique) ; le lanceur ne montre le champ que sous ces valeurs et ne l'envoie pas sinon |
...
| ce qu'une CHARTE impose à une IMAGE créée (palette à la place de celle choisie, mots de style, interdits, police et couleurs de titre, logo posé, texture, constat sur l'image) | la chaîne : l'étape `contrainte` EN PREMIER (graphe `image-heraldiste`), la `direction` qui reçoit `positif`, `couleurs_en`, `negatif`, `logo_ancrage` du récit, la `livraison` (composer : `textes`, `images`, `texture`), l'étape `conformite` (graphe `video-heraldiste-conformite`, `1.video` = le chemin de l'image livrée, `1.images` = 1), les constats de `constat_de_la_charte` ; et dans chaque technique `negatif.applique` (le négatif est-il lu ?) | le runner ; `DirectionDImage` (récit `charte_palette`, `palette_dite`, `zone_du_logo`) ; `HeraldisteCharte` (récit `positif`, `couleurs_en`) |
