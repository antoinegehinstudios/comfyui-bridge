...
def test_un_bloc_vise_le_role_du_precedent_pas_son_numero():
    # « les images » plutot que « le noeud 13 » : le fragment peut changer sa
    # plomberie sans casser la chaine.
    plan = [
        {"fragment": "amorce", "sorties": {"images": "9"},
         "contenu": {"9": {"class_type": "Decode", "inputs": {}}}},
        {"pour": {"jusqu_a": "n", "chaque": 1},
         "faire": [{"fragment": "segment", "sorties": {"images": "4"},
                    "contenu": {"4": {"class_type": "Decode",
                                      "inputs": {"src": ["$precedent.images", 0]}}}}]},
    ]
    g = assembler(deplier(plan, {"n": 2}))
    decodes = sorted(g, key=int)
    assert g[decodes[1]]["inputs"]["src"][0] == decodes[0]      # segment 0 -> amorce
    assert g[decodes[2]]["inputs"]["src"][0] == decodes[1]      # segment 1 -> segment 0


def test_un_role_inconnu_dit_ce_qui_est_offert():
    plan = [
        {"fragment": "a", "sorties": {"images": "1"},
         "contenu": {"1": {"class_type": "X", "inputs": {}}}},
        {"fragment": "b", "contenu": {
            "1": {"class_type": "Y", "inputs": {"src": ["$precedent.latentes", 0]}}}},
    ]
    with pytest.raises(WorkflowMappingError) as e:
        assembler(deplier(plan, {}))
    assert "latentes" in str(e.value)


def test_un_numero_reste_utilisable_quand_aucun_role_n_est_declare():
    plan = [
        {"fragment": "a", "contenu": {"7": {"class_type": "X", "inputs": {}}}},
        {"fragment": "b", "contenu": {
            "1": {"class_type": "Y", "inputs": {"src": ["$precedent.7", 0]}}}},
    ]
    g = assembler(deplier(plan, {}))
    ids = sorted(g, key=int)
    assert g[ids[1]]["inputs"]["src"][0] == ids[0]


def test_une_liaison_declaree_vise_aussi_un_role():
    from comfyui_bridge.adapter.assembleur import adresse, numeroter, sorties_nommees
    plan = [{"fragment": "commun", "sorties": {"prompt": "26"},
             "contenu": {"26": {"class_type": "Texte", "inputs": {"value": ""}}}}]
    frags = deplier(plan, {})
    assert adresse("$commun.prompt", numeroter(frags), sorties_nommees(frags)) == "1"


def test_le_nom_du_morceau_porte_le_rang_du_bloc():
    # L'ordre des morceaux doit etre dans le NOM : se fier a l'ordre ou le
    # moteur a fini d'ecrire, c'est se fier a son ordonnanceur.
    plan = [{"pour": {"jusqu_a": "n", "chaque": 1},
             "faire": [{"fragment": "b", "contenu": {
                 "1": {"class_type": "Ecrire",
                       "inputs": {"prefixe": {"$texte": "morceaux/bloc",
                                              "$rang": "tour + 1"}}}}}]}]
    g = assembler(deplier(plan, {"n": 3}))
    assert sorted(v["inputs"]["prefixe"] for v in g.values()) == [
        "morceaux/bloc_001", "morceaux/bloc_002", "morceaux/bloc_003"]


def test_un_bloc_connait_son_rang_et_le_total_des_morceaux():
    # L'amorce est hors boucle et le premier tour vaut 0 : ni l'un ni l'autre
    # ne dit à un bloc où il en est dans le récit. « blocs » le dit.
    amorce = {"fragment": "amorce", "contenu": {
    ...
    segment = {"fragment": "segment", "contenu": {
        "1": {"class_type": "Consigne", "inputs": {"rang": {"$calc": "bloc_rang"},
                                                    "total": {"$calc": "blocs_total"}}}}}
    plan = [COMMUN, amorce,
            {"pour": {"jusqu_a": "n", "chaque": 1, "deja": 1}, "faire": [segment]}]
    g = assembler(deplier(plan, {"n": 4}), {}, blocs=["amorce", "segment"])
    vus = sorted((v["inputs"]["rang"], v["inputs"]["total"])
                 for v in g.values() if v["class_type"] == "Consigne")
    assert vus == [(0, 4), (1, 4), (2, 4), (3, 4)]


def test_un_bloc_declare_mais_absent_du_montage_se_lit():
    plan = [COMMUN, AMORCE]
    with pytest.raises(WorkflowMappingError) as e:
