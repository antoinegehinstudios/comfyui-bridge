...
| `video-revelation-brume.json` | « Révéler une image par la brume », l'ESSAI d'une autre technique dans la même catégorie : même amont (`analyse` → `culture` → `intention` → `plan_valide`), un `deroulement` par `video-reveal-brume-dirige` (la peinture est là, la brume s'en va), et — depuis le 2026-09-15 — la MÊME queue que l'encre : `raccord`, `conclusion` par `video-reveal-brume-closing` (la brume revient, le climax en dernier), `appel`, `montage` à deux parts, `controle`. Onze étapes ; `contemplation_s` et `conclusion_s` comme l'encre ; `fond` choisit la teinte de la brume |
