...
| `video-revelation-brume.json` | « Révéler une image par la brume », l'ESSAI d'une autre technique dans la même catégorie : même amont (`analyse` → `intention` → `plan_valide`), seule la peinture change — la révélation est en couleur sous une nappe qui se dissipe. Huit étapes : ni `raccord` ni `conclusion` (une brume qui reviendrait ne refermerait rien), l'`appel` part du déroulement, le `montage` n'a qu'une part. Menus `style_narratif`, `style_approche`, `fond` (les trois brumes), champ `cta` |
