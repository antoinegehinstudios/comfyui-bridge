...
| `blocs/livrer.json`, `blocs/segment-h3-texte.json` | LIVRER un bloc rendu (ancre + nouvelles → FILM ×5 → une image sur N à phase globale → agrandissement ×2 commutable → lanczos → VHS ; offre `ecrit` et `derniere_image`) ; le maillon de rendu dirigé qui offre `ancre`, `nouvelles`, `derniere_image` |
