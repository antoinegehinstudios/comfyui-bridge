...
|---|---|
| `video-h3-texte.json` | MiniMax H3 **sans image de départ**, DIRIGÉ : `DirectionDeStyle` dans le commun, `DirectionDuBloc` dans l'amorce et le maillon (chaque bloc joue son temps du récit) ; **deux runs du moteur par bloc** (`un_run_par_tour`, `phases: 2`) : la phase 0 ENCODE (au bloc 0 l'amorce — texte seul ou références —, ensuite `segment-h3-encodage` depuis la dernière image relayée : DirectionDuBloc + nœud MiniMax → conditionnement et latent écrits en fichiers, avec le seul encodeur de texte), la phase 1 REND et LIVRE (`rendu` : conditionnement et latent relus, le modèle seul, échantillonneur, décodage ; puis `livrer` : cadence FILM, RealESRGAN ×2, lanczos, écriture en trois tiers gardés) ; le commun est séparé en `texte` (CLIP), `modele` (UNET, LoRA, ordonnanceur) et `commun`, chaque run ne posant que ce qu'il cite ; trois relais (`derniere_image` SaveImage/LoadImage, `conditionnement` SauverConditionnement/ChargerConditionnement, `latent` SauverLatent/ChargerLatent — emboîté vidéo + son) portent d'un run à l'autre, et `blocs` nomme `livrer` pour les rangs. Boucle comptée en secondes natives (`deja` = la première image), `blocs_total` calculé par le commun. Jusqu'à trois IMAGES DE RÉFÉRENCE commentées (`image`, `role_image`…) : au tour 0, l'amorce devient « texte + références → vidéo » (`MiniMaxH3ReferenceToVideo`, étiquettes `<Picture i>`), une variante par nombre d'images, sous le même nom. Mesuré le 2026-09-18 : envoyé entier, le montage tenait ses modèles et tous ses blocs dans un seul prompt, sans point de contrôle — le moteur est mort au bloc 3. C'est la chaîne `video-depuis-un-texte` qui le publie |
| `blocs/livrer.json`, `blocs/segment-h3-encodage.json` (et `segment-h3-texte.json`, le maillon d'un seul tenant) | LIVRER un bloc rendu en trois tiers de 41 images natives, chacun attendant l'écriture du précédent et gardé par une GARDE DE PLACE (`GardeDePlace`, paquet comfyui-garde-de-place : la marge du poste, RAM et commit, doit tenir `garde_go` Gio avant d'entrer dans le tier — sinon attente, puis refus propre) (ancre du tier + nouvelles → FILM ×5 → une image sur N à phase globale → agrandissement ×2 commutable → lanczos → VHS, morceau 3·rang + tier ; offre `ecrit` et `derniere_image`) ; le maillon de rendu dirigé qui offre `ancre`, `nouvelles`, `derniere_image` |
