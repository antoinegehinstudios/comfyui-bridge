...
|---|---|
| `video-h3-texte.json` | MiniMax H3 **sans image de départ**, DIRIGÉ : `DirectionDeStyle` dans le commun, `DirectionDuBloc` dans l'amorce et le maillon (chaque bloc joue son temps du récit) ; **un run du moteur par tour** (`un_run_par_tour`) — au tour 0 l'amorce (`si tour == 0`), ensuite `segment-h3-texte` (rendu seul), puis `livrer` (cadence FILM, RealESRGAN ×2, lanczos, écriture en trois tiers) ; le relais `derniere_image` (SaveImage en queue du run, LoadImage au début du suivant) porte la dernière image native d'un run à l'autre, et `blocs` nomme `livrer` pour les rangs. Boucle comptée en secondes natives (`deja` = la première image), `blocs_total` calculé par le commun. Jusqu'à trois IMAGES DE RÉFÉRENCE commentées (`image`, `role_image`…) : au tour 0, l'amorce devient « texte + références → vidéo » (`MiniMaxH3ReferenceToVideo`, étiquettes `<Picture i>`), une variante par nombre d'images, sous le même nom. Mesuré le 2026-09-18 : envoyé entier, le montage tenait ses modèles et tous ses blocs dans un seul prompt, sans point de contrôle — le moteur est mort au bloc 3. C'est la chaîne `video-depuis-un-texte` qui le publie |
| `blocs/livrer.json`, `blocs/segment-h3-texte.json` | LIVRER un bloc rendu en trois tiers de 41 images natives, chacun attendant l'écriture du précédent et gardé par une GARDE DE PLACE (`GardeDePlace`, paquet comfyui-garde-de-place : la marge du poste, RAM et commit, doit tenir `garde_go` Gio avant d'entrer dans le tier — sinon attente, puis refus propre) (ancre du tier + nouvelles → FILM ×5 → une image sur N à phase globale → agrandissement ×2 commutable → lanczos → VHS, morceau 3·rang + tier ; offre `ecrit` et `derniere_image`) ; le maillon de rendu dirigé qui offre `ancre`, `nouvelles`, `derniere_image` |
