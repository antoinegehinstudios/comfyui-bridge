...
def adresse(reference: str, table: dict[tuple[str, int], dict[str, str]]) -> str:
...
def _cible(reference: str, moi: tuple[str, int], ordre: list[tuple[str, int]],
           table: dict[tuple[str, int], dict[str, str]]) -> str:
    ...
    if local not in locaux:
        raise WorkflowMappingError(
            f"fragment {moi[0]!r} : le nœud {local!r} n'existe pas dans {vise[0]!r}")
    return locaux[local]
...
def assembler(fragments: list[Fragment],
              constantes: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for f in fragments:
        ...
        for local, noeud in (f.contenu or {}).items():
            ...
            for champ, valeur in (noeud.get("inputs") or {}).items():
                if _est_lien(valeur):
                    entrees[champ] = [_cible(valeur[0], moi, ordre, table), valeur[1]]
                else:
